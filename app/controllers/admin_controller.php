...
class AdminController extends AppController {
	...
	function admin_index()
	{
	 ...
	 $this->checkPermissionOwner();
	  ...
	  $properties = $this->Property->find('all',array('conditions'=>array('owner_id'=>$this->Session->read('user.id')),'fields'=>array('admin_id')));   	 
 
	 $in_array = array('-1');
  foreach ($properties as $val)
  {	  array_push($in_array,$val['Property']['admin_id']);  }
  $in_array = array_unique($in_array);
  
	$this->Admin->recursive  = 0;	
	$this->paginate['Admin'] = array(  'fields'      => array('Admin.id','Admin.firstname','Admin.lastname','Admin.email'),
                                    'conditions'  => array('Admin.id IN('.implode(',',$in_array).')'));
...
function admin_adminedit($id = null) {
  ...
  if($result['Admin']['imagename']!="")
  {
	...
	if (!empty($this->data))
	{
		...
		$getUser = $this->Admin->find('first', array('conditions' => array('Admin.username'=> $username, 'Admin.password'=> $password, 'Admin.enable'=> 1), 'fields' => array('id', 'username', 'email','userlevel', 'created','last_login')));
