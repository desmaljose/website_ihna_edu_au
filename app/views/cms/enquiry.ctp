				...
				<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 no-padding">
      				<h1 class=" wow triggerAnimation animated" data-animate="fadeInLeft"  data-wow-delay=".1s">Enquiry Form</h1><br />
...
<!-- form goes here -->
<form class="form-horizontal" method="post" name="frm" id="frm" action="<?php echo WEB_URL.'cms/enquiry/'; ?>" enctype="multipart/form-data">
    <input type="hidden" name="final_submit" id="final_submit" value="0" />
  <div class="form-group">
      <label for="course_id" class="col-sm-2 control-label">Course Name <span>*</span></label>
    <div class="col-sm-10">
        <select name="course_id" class="form-control" id="course_id" onchange="$('#campus_id').empty(); $('#student_type_id').empty(); $('#mod_of_delivery_id').empty(); $('#start_date_id').empty(); $('#course_name').val(this.options[this.selectedIndex].text); document.frm.submit();">
          <option value="">Select</option>
<?php foreach($courses as $row){ ?>
          <option value="<?php echo $row['c']['id'] ?>" <?php if($row['c']['id']==@$course_id){ echo ' selected'; } ?>><?php echo $row['c']['course_name'] ?></option>
          <?php } ?>
  </select> 
    ...
    <div class="col-sm-10">
        ...
        <select name="campus_id" class="form-control" id="campus_id" onchange="$('#student_type_id').empty(); $('#mod_of_delivery_id').empty(); $('#start_date_id').empty(); $('#campus_name').val(this.options[this.selectedIndex].text); document.frm.submit();">
          <option value="">Select</option>
<?php foreach($campuses as $row){ ?>
    ...
    <div class="col-sm-10">
      ...
      <select name="student_type_id" class="form-control" id="student_type_id" onchange="$('#mod_of_delivery_id').empty(); $('#start_date_id').empty(); $('#student_type_name').val(this.options[this.selectedIndex].text); document.frm.submit();">
          <option value="">Select</option>
<?php foreach($student_types as $row){ ?>
          <option value="<?php echo $row['st']['id'] ?>" <?php if($row['st']['id']==@$student_type_id){ echo ' selected'; } ?>><?php echo $row['st']['name'] ?></option>
          <?php } ?>
  </select>
        
        
        <input type="hidden" name="student_type_name" id="student_type_name" value="<?php echo @$student_type_name; ?>" />
...
<div class="radio">
  <label>
    ...
    <div class="col-sm-10">
        <input type="state" class="form-control" name="state" id="state" value="<?php echo @$state; ?>">
    </div>
  </div>
      <div class="form-group">
...
<div class="radio">
  <label>
    <input type="radio" name="aus" id="aus_yes" value="Yes" <?php if(@$aus=='Yes'){ echo ' checked';} ?>>
    Yes
  </label>
</div>
<div class="radio">
  <label>
    <input type="radio" name="aus" id="aus_no" value="No" <?php if(@$aus=='No'){ echo ' checked';} ?>>
    No
  </label>
</div>
          </div>
    ...
    <div class="col-sm-10">
        <textarea name="enquiry" id="enquiry" class="form-control" rows="3" style="resize:vertical;"><?php echo @$enquiry; ?></textarea>
    </div>
  </div>
