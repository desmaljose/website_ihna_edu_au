...
<div class="mainBox subBoxborder">
      ...
      <div class="block_head">
       <h1 class="fleft"><?php __('Edit Content');?></h1>
...
<?php echo $form->create('Cm');?>
	...
	<?php
        ...
        <div>
        <select id="parent_id" name="data[Cm][parent_id]">
          <option value="0">---Main Page---</option>
          <?php if($pages){
          foreach($pages as $pkey =>$pval){
            ?>
            <option value="<?php echo $pval['Cm']['id'];?>" <?php if($this->data['Cm']['parent_id']==$pval['Cm']['id']){?>selected<?php }?>><?php echo $pval['Cm']['page_title'];?></option>
            ...
            if($check_subpage_exists){
              ...
              {
            ?><option value="<?php echo $sval['Cm']['id'];?>" <?php if($this->data['Cm']['parent_id']==$sval['Cm']['id']){?>selected<?php }?>>&nbsp;&nbsp;&nbsp;<?php echo $sval['Cm']['page_title'];?></option>
            <?php }?>
            <?php
              }
             }
      ...
      <?php  //echo $form->input('parent_id',array('type'=>'select','empty'=>'--Main Page--','options'=>$pages));
